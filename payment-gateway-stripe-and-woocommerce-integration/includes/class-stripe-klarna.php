...
class EH_Klarna_Gateway extends WC_Payment_Gateway {
    ...
    public function get_charge_details( $wc_order, $client, $currency, $amount, $payment_method) {
        ...
        $charge['return_url'] = add_query_arg(
            ...
            WC()->api_request_url('EH_Klarna_Gateway')
        );
    ...
    public function eh_klarna_callback_handler() {
        ...
        if ( isset($_REQUEST['payment_intent']) && ! empty($_REQUEST['payment_intent']) ) {
            ...
            if ( empty($saved_intent_id) || ! hash_equals($saved_intent_id, $intent_id) ) {
                ...
                ), get_bloginfo('blogname') . ' - Klarna: intent mismatch - Order #' . $order_id);
            ...
            try {
                ...
                // Klarna is primarily webhook-driven — inline processing is opt-in
                if ( true === apply_filters('wt_stripe_klarna_processing', true) ) {
                ...
                EH_Stripe_Log::log_update('dead', array(
                    'order_id' => $order_id,
                    'error'    => $e->getMessage(),
                ), get_bloginfo('blogname') . ' - Klarna: intent retrieve failed - Order #' . $order_id);
        ...
        // No payment_intent — Klarna is webhook-driven, don't assume failure
        if ( $order->has_status(array('processing', 'completed')) ) {
            wp_safe_redirect($this->get_return_url($order));
        } else {
    ...
    public function eh_process_payment_response( $response, $order = null ) {
        ...
        if(isset($response->status) && 'succeeded' === $response->status){
            if (isset($charge_response->paid) && true === $charge_response->paid) {
                ...
                if(isset($charge_response->captured) && true === $charge_response->captured){
                    if ( ! $order->has_status( array( 'processing', 'completed' ) ) ) {
